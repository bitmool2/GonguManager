...
export interface PlanConfig {
  ...
  maxOrdersPerMonth: number;  // -1 = 무제한 (pass형은 passLimit)
  passTotal?: number;
  passOrderLimit?: number;    // pass형: 회당 주문 한도
  passExpiryDays?: number;
  features: {
    autoPaymentMatch: boolean;
    kakaoNotification: number;  // 건 (-1 = 무제한, 0 = 없음)
    excelDownload: boolean;
    buyerCRM: boolean | 'view';
    aiAnnouncement: number;     // 회 (-1 = 무제한, 0 = 없음)
    projectCopy: boolean;
  };
}
